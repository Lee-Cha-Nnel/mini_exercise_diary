from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, 
                             ...
                             QDialog) # ⭐ QDialog(팝업창 용도) 임포트 추가!
from PyQt5.QtCore import Qt, pyqtSignal, QDate
from PyQt5.QtGui import QColor

from db_supabase import WorkoutDB

# 🌟 새로 추가됨: 실시간으로 DB에서 데이터를 가져오는 팝업 창 클래스
class DietGuideDialog(QDialog):
    ...
    def initUI(self):
        ...
        title.setStyleSheet("font-size: 18px; font-weight: bold; color: #2D3748; margin-bottom: 10px;")
        ...
        # 🚀 수파베이스 DB에서 가이드 데이터를 싹 가져옵니다!
        guide_data = self.db.get_diet_guide()
        
        if not guide_data:
            self.table.setRowCount(1)
            self.table.setItem(0, 0, QTableWidgetItem("데이터를 불러오지 못했거나 비어있습니다."))
        else:
        ...
        close_btn.setStyleSheet("background-color: #4A5568; color: white; padding: 8px; border-radius: 5px; font-weight: bold;")
...
class DietWindow(QWidget):
    ...
    def initUI(self):
        ...
        # 🌟 새로 추가됨: 식단 가이드 버튼을 입력 폼 바로 위에 배치
        guide_layout = QHBoxLayout()
        guide_layout.addStretch()
        self.btn_guide = QPushButton("📋 식단 가이드 보기")
