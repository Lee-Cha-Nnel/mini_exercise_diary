...
class WorkoutDB:
    ...
    # 🌟 새로 추가: DB에서 식단 가이드 목록 가져오기
    def get_diet_guide(self):
        try:
            # diet_guide 테이블의 모든 데이터를 이름(food_name) 순서대로 가져옴
            res = self.supabase.table('diet_guide').select('*').order('food_name').execute()
            return res.data
